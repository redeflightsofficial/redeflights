...
create policy "Public can read active umrah packages" on public.umrah_packages for select to anon, authenticated using (status = 'active');
drop policy if exists "Admin can manage umrah packages" on public.umrah_packages;
create policy "Admin can manage umrah packages" on public.umrah_packages for all to anon, authenticated using (true) with check (true);
